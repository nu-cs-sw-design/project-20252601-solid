...
import domain.game.CardPlayService;
import domain.game.CardType;
import domain.game.Game;
import domain.game.GameType;
import domain.game.IGamePrompts;
...
public class GameUI implements IGamePrompts {
	...
	public GameUI(Game game) {
		this.game = game;
		this.cardPlayService = new CardPlayService(game, this);
	}

	// ---------------------------
	// IGamePrompts implementation
	// ---------------------------

	@Override
	public int askExplodingKittenInsertIndex(int maxIndex) {
		...
		final String validRangeMessage = MessageFormat.format(
				messages.getString("validRangeMessage"), maxIndex);
	...
	@Override
	public void showTopCards(Player player, List<Card> cards) {
		final String futureCards = messages.getString("futureCards");
		final String futureCardTemplate = messages.getString("futureCard");
		String header = MessageFormat.format(futureCards, cards.size());
		System.out.println(header);
		for (Card c : cards) {
			String msg = MessageFormat.format(
					futureCardTemplate,
					getLocalizedCardType(c.getCardType()));
	...
	@Override
	public boolean askPlayerWantsToPlayNope(Player reactingPlayer, Card originalCard) {
		Scanner scanner = new Scanner(System.in, StandardCharsets.UTF_8);
		final String playerHasNopeCard = messages.getString("playerHasNopeCard");
		...
		System.out.println(MessageFormat.format(
				wouldYouPlayNope,
				getLocalizedCardType(originalCard.getCardType())));
		...
		while (hasCard(tempTurn, CardType.ATTACK)
			...
			if (hasCard(tempTurn, CardType.ATTACK)) {
				...
				final String hasTargetedAttackPrompt = MessageFormat.format(
						messages.getString("hasTargetedAttackPrompt"), tempTurn);
				final String yesOption = messages.getString("optionYes");
				final String noOption = messages.getString("optionNo");
				final String invalidChoiceMessage =
						messages.getString("invalidChoice");
			...
			if (!player.getIsDead()) {
				...
				if (player.getIsCursed()) {
					...
					for (int index = 0; index < player.getHandSize(); index++) {
						System.out.print(
								getLocalizedCardType(
										player.getCardAt(index).getCardType()));
					}
					System.out.println();
					player.shuffleHand();
				}
	...
	private void printPlayerTurn() {
		...
		if (checkIfPlayerIsCursed(player)) {
			...
			System.out.println(messages.getString("playerHand"));
			for (int handIndex = 0; handIndex < getHandSize(currentPlayer); handIndex++) {
				String unknown = messages.getString("unknownCard");
				System.out.println("  [" + handIndex + "] " + unknown);
			}
		} else {
			...
			for (int handIndex = 0; handIndex < getHandSize(currentPlayer); handIndex++) {
				String label = getLocalizedCardType(
						game.getCardType(currentPlayer, handIndex));
				System.out.println("  [" + handIndex + "] " + label);
			}
		}
					...
					if (checkIfCardMarked(playerIndex, cardIndex)) {
						final String markedCardMessage =
								MessageFormat.format(
										...
										playerIndex,
										cardIndex,
										game.getCardType(playerIndex, cardIndex));
					...
					while (true) {
						...
						switch (userInputTwo) {
							case "2":
								...
								return twoCombo;
							case "3":
								System.out.println(specialComboThree);
								return threeCombo;
							default:
			...
			if (playerCounter != playerIndex) {
				if (game.checkIfPlayerHasCard(playerCounter, CardType.NOPE)) {
					final String hasNopeCard = MessageFormat.format(
							messages.getString("playerHasNopeCard"),
							playerCounter);
	...
	private boolean playExplodingKitten(int playerIndex) {
		...
		final String explodingKittenMessage =
				messages.getString("explodingKittenMessage");
		final String noDefuseCardMessage =
				messages.getString("noDefuseCardMessage");
		final String youExplodedMessage =
				messages.getString("youExplodedMessage");
		final String defusedMessage =
				messages.getString("defusedMessage");
		final String whereToInsertMessage =
				messages.getString("whereToInsertMessage");
		final String validRangeMessage = MessageFormat.format(
				messages.getString("validRangeMessage"), game.getDeckSize());
		final String invalidInputMessage =
				messages.getString("invalidInputMessage");
		final String cursedMessage =
				messages.getString("cursedExplodingMessage");
		final String notDefuseCardMessage =
				messages.getString("notDefuseCardMessage");
		final String discardCardMessage =
				messages.getString("discardCardMessage");
		final String reenterDefuseMessage =
				messages.getString("reenterDefuseMessage");
	...
	private boolean endTurn() {
		...
		final String streakingKittenMessage =
				messages.getString("streakingKittenMessage");
		final String invalidNumber = messages.getString("invalidNumber");

		if (checkMatchingCardType(cardDrawn.getCardType(),
				CardType.EXPLODING_KITTEN)) {
	...
	private void playCurseOfTheCatButt() {
		final String decidedToPlayCard =
				messages.getString("decidedToPlayCatButtCard");
		final String enterPlayerIndex =
				messages.getString("enterPlayerIndexCurse");
		final String invalidPlayerIndex =
				messages.getString("invalidPlayerIndex");
		final String playerExploded =
				messages.getString("playerExplodedChooseNew");
		final String cannotCurseSelf =
				messages.getString("cannotCurseSelf");
		final String enterValidInteger =
				messages.getString("enterValidInteger");
		final String playerCursed =
				messages.getString("playerCursed");
	...
	private void playSpecialComboTwoCards(CardType cardType) {
			...
			try {
				...
				if (checkUserOutOfBounds(playerIndex)) {
					final String validPlayerIndexMessage =
							enterValidPlayerIndex + (game.getNumberOfPlayers() - 1);
		...
		try {
			...
			if (checkMatchingCardType(stealedCard.getCardType(),
				...
				int explodingKittenIndex = game.getIndexOfCardFromHand(
	...
	private void drawFromTheBottom() {
		final String drewFromBottomMessage =
				messages.getString("drewFromBottom");
		...
		final String cardType =
				game.getCardType(game.getPlayerTurn(),
								game.getHandSize(game.getPlayerTurn()) - 1)
						.toString();

		final String formattedCardMessage =
				MessageFormat.format(cardWasMessage, cardType);
		System.out.println(formattedCardMessage);
	}

	private void playMark() {
		final String decidedToPlayMark =
				messages.getString("decidedToPlayMark");
		final String enterPlayerToMark =
				messages.getString("enterPlayerToMark");
		...
		final String cannotMarkSelf =
				messages.getString("cannotMarkSelf");
		final String enterCardToMark =
				messages.getString("enterCardToMark");
		final String enterInteger =
				messages.getString("enterInteger");
	...
	private void playCatomicBomb() {
		...
		final String playCatomicBomb =
				messages.getString("playCatomicBomb");
		final String explodingKittensTop =
				messages.getString("explodingKittensTop");
				...
				switch (index) {
					case explodingKittenAtTop:
						formattedMessage = MessageFormat.format(
								messages.getString("cardExplodingKittenTh"),
								index);
						break;
					case explodingKittenSecondFromTop:
						formattedMessage = MessageFormat.format(
								messages.getString("cardExplodingKittenSt"),
								index);
						break;
					case explodingKittenThirdFromTop:
						formattedMessage = MessageFormat.format(
								messages.getString("cardExplodingKittenNd"),
								index);
						break;
					case explodingKittenFourthFromTop:
						formattedMessage = MessageFormat.format(
								messages.getString("cardExplodingKittenRd"),
								index);
						break;
					default:
						formattedMessage = "";
	...
	private void playSpecialComboThreeCards(CardType cardType) {
		...
		final String enterValidInteger =
				messages.getString("enterValidInteger");
		final String enterPlayerToSteal =
				messages.getString("enterPlayerToSteal");
		final String invalidPlayerIndex =
				messages.getString("invalidPlayerIndex");
		final String playerExploded =
				messages.getString("playerExploded");
		final String cannotStealFromSelf =
				messages.getString("cannotStealFromSelf");
		final String enterCardTypeToSteal =
				messages.getString("enterCardTypeToSteal");
		final String selectedCardType =
				messages.getString("selectedCardType");
		final String invalidCardType =
				messages.getString("invalidCardType");
	...
	private void playSkip(boolean superSkip) {
		...
		final String turnsRemainingTemplate =
				messages.getString("turnsRemaining");
		final String formattedTurnsRemaining;

		System.out.println(decidedSkip);
		int turnsLeft = game.playSkip(superSkip);
		formattedTurnsRemaining = MessageFormat.format(
				turnsRemainingTemplate, turnsLeft);
		System.out.println(formattedTurnsRemaining);
	}

	private void playAlterTheFuture() {
		final String alterFutureMessage =
				messages.getString("alterFutureMessage");
		final String futureCardsMessage =
				messages.getString("futureCardsMessage");
		final String enterNewOrderMessage =
				messages.getString("enterNewOrderMessage");
		final String positionPrompt =
				messages.getString("positionPrompt");
		final String positionTakenMessage =
				messages.getString("positionTakenMessage");
		final String invalidPositionMessage =
				messages.getString("invalidPositionMessage");
		final String futureAlteredMessage =
				messages.getString("futureAlteredMessage");
		final String cardDisplayFormat =
				messages.getString("cardDisplayFormat");
		...
		for (int i = 0; i < cardsToReveal; i++) {
			final String positionRequest =
					MessageFormat.format(positionPrompt, i + 1);
	...
	private boolean checkIfCatCard(CardType cardType) {
		return cardType == CardType.CAT_ONE
				|| cardType == CardType.CAT_TWO
				|| cardType == CardType.CAT_THREE
				|| cardType == CardType.CAT_FOUR;
	}
	...
	private boolean playImplodingKitten(Card card) {
		...
		final String invalidGreaterIndexMessage =
				messages.getString("invalidGreaterIndex");
	...
	public void startTurn() {
		...
		final String enterValidIntegerMessage =
				messages.getString("enterValidInteger");
		final String notEnoughCardsComboMessage =
				messages.getString("notEnoughCardsComboMessage");
		final String notEnoughCardsMessage =
				messages.getString("notEnoughCardsMessage");
		final String howManyFeralCatsMessage =
				messages.getString("howManyFeralCats");
		final String invalidFeralCatsNumberMessage =
				messages.getString("invalidFeralCatsNumber");
		final String invalidCardTypeMessage =
				messages.getString("invalidCardTypeMessage");
		...
		while (!checkIfTheyEndTurn()) {
			...
			// For our 3 refactored cards, delegate to CardPlayService
			if (cardType == CardType.EXPLODING_KITTEN
					|| cardType == CardType.SEE_THE_FUTURE
					|| cardType == CardType.SHUFFLE) {
				cardPlayService.playCard(player, cardIndex);
				// After playing these, continue the turn loop
				continue;
			}

			if (checkIfPlayerIsCursed(player)
					&& checkMatchingCardType(cardType, CardType.STREAKING_KITTEN)) {
			...
			if (specialCombo == 0
					&& checkCardIfInvalid(player.getCardAt(cardIndex))) {
			...
			if (specialCombo == threeCats) {
				...
				if (checkIfCatCard(cardType) && numberOfFeralCats > 0) {
					...
					if (numberOfFeralCats + numberOfCatType >= threeCats) {
						...
						try {
							...
							System.out.println(enterValidIntegerMessage);
							scanner.next();
						}
						for (int i = 0; i < numberOfFeralCatsToPlay; i++) {
							player.removeCardFromHand(
									player.getIndexOfCard(CardType.FERAL_CAT));
						}
						for (int i = 0;
				...
				if (catCard && numberOfFeralCats > 0) {
					...
					if (numberOfFeralCats + numberOfCatType >= twoCats) {
						...
						for (int i = 0; i < twoCats - numberOfFeralCatsToPlay; i++) {
							player.removeCardFromHand(
									player.getIndexOfCard(cardType));
						}
					} else {
	...
	private boolean checkCardWithinBoundsIndexed(int cardIndex, int playerIndex) {
		return cardIndex >= 0 && cardIndex < game.getHandSize(playerIndex);
	}
	...
	private boolean checkExplodingKitten(int playerIndex) {
		final String invalidPlayerIndexExplodingKitten =
				messages.getString("invalidPlayerIndexExplodingKitten");
	...
	private boolean checkNumberOfCards(Player player,
									   CardType cardtype,
									   int threshold) {
